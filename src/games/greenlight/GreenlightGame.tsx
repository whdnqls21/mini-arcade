...
// 사람의 반응 하한(ms). 이보다 빠르면 '반응'이 아니라 예측/연타 → 성급(실패) 처리.
const MIN_REACTION = 100;

// 라운드 진행 상태.
//  ready   : 시작 전(StartGate)
//  waiting : 빨강 — 초록을 기다리는 중(누르면 성급 = 실패)
//  go      : 초록 — 지금 탭!
//  early   : 성급한 탭 → 판 전체 실패, 처음부터 다시 안내
...
export default function GreenlightGame({ onGameOver, bestScore, submitting }: GamePlayProps) {
  ...
  // 성급(빨강에 탭 or 하한 미만) → 판 전체 실패. 라운드 기록을 지우고 처음부터 다시 안내.
      ...
      <div className="relative mx-auto w-full max-w-[22rem]">
        ...
        {phase === "early" && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-xl bg-black/70">
            ...
            <p className="text-sm text-ink-dim">성급하게 눌러서 실패했어요.</p>
            <p className="text-xs text-danger">처음(1라운드)부터 다시 시작합니다.</p>
            <button
              onClick={begin}
